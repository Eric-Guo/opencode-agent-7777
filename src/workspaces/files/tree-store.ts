...
export function createFileTreeStore(options: TreeStoreOptions) {
  ...
  const listDir = (input: string, opts?: { force?: boolean }) => {
    ...
    const promise = options
      .list(dir)
      .then((nodes) => {
        if (generation !== version || options.scope() !== directory || inflight.get(dir) !== promise) return
        const prevChildren = tree.dir[dir]?.children ?? []
        const nextChildren = nodes.map((node) => node.path)
        const nextSet = new Set(nextChildren)
        const removedDirs = prevChildren.filter((path) => !nextSet.has(path) && tree.node[path]?.type === "directory")

        setTree(
          "dir",
          produce((draft) => {
            for (const path of Object.keys(draft)) {
              if (!removedDirs.some((removed) => path === removed || path.startsWith(removed + "/"))) continue
              delete draft[path]
