...
import type { useOpenInApp } from "@/session/files/open-in-app"
  ...
  return (
    <Dynamic
      ...
      onDragStart={(event: DragEvent) => {
        ...
        event.dataTransfer?.setData("text/uri-list", pathToFileUrl(local.node.absolute))
...
export default function FileTreeV2(props: {
  ...
  const [elements, setElements] = createStore<{ root?: HTMLDivElement; focused?: string }>({})
  const root = () => elements.root
  const setRoot = (root: HTMLDivElement) => setElements("root", root)
  const focused = () => elements.focused
  const setFocused = (path: string | undefined) => setElements("focused", path)
  ...
  createEffect(() => {
    if (!live()) return
    if (file.directory()) void file.tree.list("")
  })
                    ...
                    <Show
                      ...
                      fallback={
                        <OpenInAppContextMenuV2
                          state={openIn}
                          path={() =>
                            resolveOpenInAppPath(file.directory(), row().node.absolute || row().node.originalPath)
                          }
                        >
                          <FileTreeNodeV2
                            node={{
                              ...row().node,
                              absolute: resolveOpenInAppPath(
                                file.directory(),
                                row().node.absolute || row().node.originalPath,
                              ),
                            }}
                            level={row().level}
                            active={active()}
