...
describe("createRequestQueue", () => {
  ...
  test("never counts the event stream against the budget", async () => {
    ...
    const requests = [input.queue.fetch("http://server/api/session"), input.queue.fetch("http://server/api/event")]
    await input.settle()
  ...
  test("aborted requests leave the queue without being sent", async () => {
    const input = setup({ limit: 1 })
    const controller = new AbortController()
    const first = input.queue.fetch("http://server/api/first")
  ...
  test("releases slots when a custom transport throws synchronously", async () => {
    let calls = 0
    const queue = createRequestQueue({
      limit: 1,
      fetch: Object.assign(
        () => {
          if (calls++ === 0) throw new Error("transport failed")
          return Promise.resolve(new Response("ok"))
        },
        { preconnect() {} },
      ),
    })
    await expect(queue.fetch("http://server/api/first")).rejects.toThrow("transport failed")
    await expect(queue.fetch("http://server/api/next")).resolves.toBeInstanceOf(Response)
    expect(queue.inflight()).toBe(0)
  })

  test("releases slots after headers without waiting for a streaming response body", async () => {
    const input = setup({ limit: 1 })
    const first = input.queue.fetch("http://server/api/first")
    const second = input.queue.fetch("http://server/api/second")
    input.pending[0]!.resolve()
    const response = await first
    expect(response.bodyUsed).toBe(false)
    expect(input.pending).toHaveLength(2)
  ...
  test("logs what is in flight and queued once per burst after requests stall", async () => {
    const input = setup({ stallMs: 5 })
    const requests = [input.queue.fetch("http://server/api/worktree?location[directory]=%2Fa")]
    input.tick(50)
    requests.push(input.queue.fetch("http://server/api/worktree?location[directory]=%2Fb"))
    input.tick(50)
    requests.push(input.queue.fetch("http://server/api/worktree?location[directory]=%2Fc"))
    input.tick(100)
    requests.push(input.queue.fetch("http://server/api/info"))
    expect(input.logs).toEqual([])
    input.tick(2_000)
    await new Promise((resolve) => setTimeout(resolve, 20))
    expect(input.logs).toEqual([
