...
| Timeline projection and rendering               | `src/pages/session/timeline/model.ts`, `src/pages/session/timeline/timeline-row.ts`, `src/pages/session/timeline/message-timeline.tsx`, `src/pages/session/timeline/summary-diffs.ts`                                                                                                                                                                                                                                                                                                                                                                             | `src/pages/session/use-session-hash-scroll-to-end.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Row construction, protocol notices, grouped assistant-part construction, deterministic projection ordering, reconciliation, context-tool grouping, copy targeting, and latest-error selection come from the shared `@opencode-ai/session-ui/timeline/projection`. 7777 keeps the latest-nine window and per-turn diff-summary cards in its model and renders rows through the shared current-message components. It still omits user-driven paging, virtualization, measurement, and the full summary-diff UI. |
...
- Timeline rows, grouping, and reconciliation come from the shared `@opencode-ai/session-ui/timeline/projection`
  (`createReactiveTimelineProjection`); the old local `rows.ts`, `projection.ts`, and `row-reconciliation.ts` parity
  copies were removed with the upstream refactor. `src/pages/session/timeline/timeline-row.ts` now holds only the
  7777-only `DiffSummary` row, and `summary-diffs.ts` still feeds it. The compact message cache retains ordered raw V2
  messages so the shared projection can preserve protocol notices, shell turns, chronologically placed optimistic
  turns, stable context-group identity, interruption boundaries, recovered streaming turns, and grouped assistant
  parts.
- `src/pages/session/timeline/message-timeline.tsx` renders the projected rows through the shared current-message
  components (`SessionUserMessage`, `SessionAssistantContent`, `SessionContextToolGroup`, `SessionShellMessage`),
  including the main app's last-text copy action, protocol notices, and per-part spacing. Protocol notices use the
  current weak-text treatment. Tools are always rendered as V2 timeline parts; the removed 7777-only tool visibility
  toggle is no longer a product gap.
...
- Main-app tests for the identical model-search, UUID, and summary-diff utilities are colocated under the same
  relative paths in 7777. Grouped-row projection and current-row construction are owned and tested by the shared
  `@opencode-ai/session-ui` timeline package; 7777's timeline model test covers only the compact window and
  diff-summary injection.
