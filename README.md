...
This review was refreshed against `packages/app` commit `c6d9296b0b4f800bf2038fa89e72e321f41db63b` on
2026-08-14. The main app's `newLayoutDesigns` setting is fixed to `true`, and its legacy page layout has
...
The refresh found 53 7777 source-tree paths with a same-relative-path counterpart in the main app, and 21 of those
files are byte-for-byte identical. Against this current target, only 15 files were identical before the changes from
this review. The six newly exact files are the permission dock, request-tree selector, timeline projection, timeline
row construction, row identity, and current-message row regression test. The previous review's 55-counterpart and
23-identical snapshot is not directly comparable: the main app subsequently removed `src/utils/question-form.ts` and
its test and changed several formerly exact V2 timeline boundaries. 7777 now follows the main app's form metadata and
protocol-value contract directly and has removed those obsolete local helper files as well. Within the reviewed areas,
no known file retains an unsuffixed main-app basename for a clearly different responsibility. The remaining gaps below
are product-scope differences rather than unresolved file moves.
...
- `src/pages/session/timeline/message-timeline.tsx` resolves those shared part groups through the same
  ...
  per-part spacing. Tools are always rendered as V2 timeline parts; the removed 7777-only tool visibility toggle is no
  longer a product gap.
...
- `src/pages/session/composer/session-request-tree.ts` now matches the main app's metadata-based form and permission
  ...
  longer duplicated locally. `src/pages/session/composer/session-permission-dock.tsx` also matches the main app and
  only offers a persistent approval when the request supplies patterns to save. Directory-wide request loading remains
  imperative in `src/context/question.ts` and `src/context/permission-sync.ts`.
