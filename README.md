...
This review was refreshed against `packages/app` commit `dc50b30a30f8a79508f028c11fd80bacf3ce9142` on
2026-08-15. The main app's `newLayoutDesigns` setting is fixed to `true`, and its legacy page layout has
...
The refresh found 55 7777 source-tree paths with a same-relative-path counterpart in the main app, and 24 of those
files are byte-for-byte identical. The previous refresh against commit `c6d9296b0b4f800bf2038fa89e72e321f41db63b`
counted 53 counterparts and 21 identical files. The counterpart count dropped to 52 when the main app deleted
`src/components/prompt-input/build-request-parts.ts` and its test in favor of projecting sent messages through
`session.inbox.*` events; 7777 now follows that boundary (see the submit/inbox bullet below), and the verbatim
`src/utils/id.ts` copy restores the counterpart count. The newly exact files in this refresh are `src/utils/id.ts`,
the permission dock (exact again after the main app reverted the portable shell permission scanner that had
introduced its save-patterns guard), and `src/context/server-session-v2-reducer.ts` with its colocated test, which
7777 now shares verbatim to stream `session.text.*`/`session.reasoning.*` deltas and other V2 session events into
the compact timeline. The main app also removed `src/utils/question-form.ts` and its test between earlier reviews,
and 7777 likewise follows the main app's form metadata and protocol-value contract directly without local helper
duplicates. Within the reviewed areas, no known file retains an unsuffixed main-app basename for a clearly different
