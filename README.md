...
This review was refreshed against `packages/app` commit `d4a5f17471bafbd0e393fc3d4f84b1d7030b915b` on
2026-08-09. The main app's `newLayoutDesigns` setting is fixed to `true`, and its legacy layout implementation has
...
The refresh found 56 7777 source-tree paths with a same-relative-path counterpart in the main app, up from 52, and 22
of those files are byte-for-byte identical, up from 14. The shared set now includes the main app's grouped
assistant-part row model, row construction, projection, reconciliation, summary-diff selection, and their focused
tests. `src/utils/server-errors.ts` and `src/utils/session-message.ts` also match the main app: 7777 recognizes the same
current V2 error envelopes, uses the same safe streaming-tool input decoder, and does not manufacture an empty text
part for file-only user messages. The compact app-specific translation fallback remains in
`src/utils/readable-error.ts`, leaving the shared server-error boundary aligned. Within the reviewed areas, no known
file retains an unsuffixed main-app basename for a clearly different responsibility. The remaining gaps below are
product-scope differences rather than unresolved file moves.
...
| Timeline projection and rendering               | `src/pages/session/timeline/model.ts`, `src/pages/session/timeline/projection.ts`, `src/pages/session/timeline/rows.ts`, `src/pages/session/timeline/timeline-row.ts`, `src/pages/session/timeline/row-reconciliation.ts`, `src/pages/session/timeline/message-timeline.tsx`, `src/pages/session/timeline/summary-diffs.ts`                                                                                                                                                                                                                                                                                    | `src/pages/session/use-session-hash-scroll-to-end.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Grouped assistant-part construction, projection, reconciliation, context-tool grouping, copy targeting, and thinking/error rows now follow the main app. Message loading follows older cursors as needed to hydrate the intentional latest-nine dialog window, but the compact renderer still omits user-driven history paging, virtualization, measurement, comment cards, and the full expandable summary-diff UI. |
...
| Shared leaf utilities                           | `src/constants/file-picker.ts`, `src/utils/comment-note.ts`, `src/utils/search-keydown.ts`, `src/utils/server-errors.ts`, `src/utils/server.ts`, `src/utils/uuid.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                           | `src/utils/readable-error.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | File-picker, search-keydown, server-error, and UUID sources are identical. Comment-note parsing has the same responsibility with a local selection type. The translated readable-error wrapper and direct single-server URL/auth helper remain compact-app boundaries.                                                                                                                                               |
...
- `src/utils/session-message.ts` now byte-for-byte matches the main app's
  ...
  compaction markers, mentions, tool metadata, streaming input decoding, and file-only user turns.
...
- `src/pages/session/timeline/timeline-row.ts`, `rows.ts`, `projection.ts`, `row-reconciliation.ts`, and
  `summary-diffs.ts` now match the main app. The compact message cache retains ordered raw V2 messages so the shared
  projection can preserve shell turns, optimistic turns, stable context-group identity, interruption boundaries,
  and grouped assistant parts.
- `src/pages/session/timeline/message-timeline.tsx` resolves those shared part groups through the same
  `ContextToolGroup` and `Part` renderers, including the main app's last-text copy action and per-part spacing. Tools
  are always rendered as V2 timeline parts; the removed 7777-only tool visibility toggle is no longer a product gap.
...
- `src/context/prompt-state.ts` owns the compact reactive draft and persistence, while `src/context/prompt.ts`
  exposes the single-session composer instance; prompt fields no longer live in the server-session store.
- `src/context/global-sync/types.ts` owns the compact sync state and timeline history types.
- `src/utils/question-form.ts` now validates question forms and maps displayed labels back to form option values,
  while `src/pages/session/composer/session-request-tree.ts` selects pending forms and permissions across the active
  session's known child tree. Directory-wide request loading remains imperative in `src/context/question.ts` and
  `src/context/permission-sync.ts`.
...
- The message cache follows opaque server cursors until it has enough dialog roots to hydrate the intentional
  latest-nine timeline window or reaches the end of history. It does not expose the main app's user-driven paging UI.
...
- Main-app tests for the identical model-search, question-form, UUID, grouped-row projection, current-row
  construction, and summary-diff utilities are now colocated under the same relative paths in 7777.
