...
Target root: `<repo-root>/packages/app`, refreshed against commit `4fb4713486` on 2026-08-29. Story-only sources and
...
or read `../app`, and the package does not depend on `@opencode-ai/app`. Shared editor sources are package-owned local
copies. 55 source files share a relative path with the main app, 22 byte-for-byte identical. Both public files with
...
| Prompt input and composer | `src/composer/composer.tsx`, `src/composer/model.ts`, `src/composer/request.ts`, `src/composer/attachments/`, `src/composer/editor/`, `src/composer/suggestions/machine.ts`, `src/composer/types.ts`, `src/composer/prompt-parts.ts`, `src/composer/comment-note.ts`, `src/composer/prompt.ts`, `src/session/composer/adapter.ts`, `src/session/composer/session-composer-region-controller.ts`, `src/session/composer/session-composer-region.tsx` | `src/composer/adapter-compact.ts`, `src/composer/state-compact.ts`, `src/composer/persistence-singleton.ts`, `src/composer/submit-compact.ts`, `src/runtime/persistence/drafts-local.ts` | The view, editor controller, request shaping, and active-session adapter follow the main-app boundaries, with a compact adapter contract and package-local multiline paste handling. Commands, context, shell mode, and routed/per-tab state remain disabled; one localStorage draft. Native listener cleanup replaces the main app's helper dependency; the unused queue control omits its animation helper. |
