...
This review was refreshed against `packages/app` commit `34ac55a68a1db850d0f6393c024a54d468a7a5af` on
2026-08-06. An unsuffixed main-app filename is treated as a claim that the responsibility matches, not merely that
the feature is related. When 7777 implements a narrower or different responsibility, it keeps the main filename
stem and adds a descriptive dash suffix. For example, `server-sdk-client.ts` is the direct-client variant of the
main app's `server-sdk.tsx`, and `new-session-controller.ts` is not presented as the main app's `new-session.tsx`
page.

The refresh found 47 7777 source paths with a same-relative-path counterpart in the main app, up from 46. The new
`src/types.ts` boundary follows the main app's package-wide domain and rendering type boundary: application code now
imports those contracts from `@/types`, leaving only that boundary and the client constructor coupled directly to
`@opencode-ai/client`. The earlier `src/utils/session-message.ts` split remains aligned with the main app's
current-message normalization boundary. `src/components/dialog-select-model-search.ts`,
`src/constants/file-picker.ts`, `src/utils/search-keydown.ts`, and `src/utils/uuid.ts` remain byte-for-byte identical
to their main-app counterparts. Within the reviewed areas, no known file retains an unsuffixed main-app basename for
a clearly different responsibility. The remaining gaps below are product-scope differences rather than unresolved
file moves.
...
| Timeline projection and rendering               | `src/pages/session/timeline/model.ts`, `src/pages/session/timeline/projection.ts`, `src/pages/session/timeline/rows.ts`, `src/pages/session/timeline/timeline-row.ts`, `src/pages/session/timeline/row-reconciliation.ts`, `src/pages/session/timeline/message-timeline.tsx`                                                                                                                                                                                                                                                                                                     | `src/pages/session/use-session-hash-scroll-to-end.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | The row/projection responsibilities match. 7777 intentionally omits routed message hashes, paged history, virtualization, measurement, and summary-diff rows.                                                                                                                                                                       |
...
| Shared leaf utilities                           | `src/constants/file-picker.ts`, `src/utils/search-keydown.ts`, `src/utils/server-errors.ts`, `src/utils/server.ts`, `src/utils/uuid.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | File-picker, search-keydown, and UUID sources are identical. Server and error helpers retain the same responsibility but only implement the direct single-server cases used by 7777.                                                                                                                                                |

The concrete responsibility splits are:

- `src/types.ts` is now the package-wide import boundary for domain and legacy rendering contracts, matching the
  main app's new boundary while retaining the client types required by 7777's direct protocol adapter.
- `src/utils/session-message.ts` now owns the same current-message-to-legacy-render-record normalization boundary as
  the main app, including agent/model switches, shell turns, compaction markers, mentions, and tool metadata.
