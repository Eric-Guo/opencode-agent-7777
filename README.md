...
| Recent sessions and session switching | `src/pages/session/recent-sessions.ts`, `src/pages/new-session.tsx`, `src/components/session/session-header.tsx`, `src/context/server-session.ts` | `src/pages/home.tsx`, `src/context/directory-sync.ts`, `src/context/tabs.tsx`, `src/components/session/session-header.tsx` | 7777 lists up to 12 recent root sessions for the active directory through the SDK and exposes them from a compact header dropdown. The app owns broader home-page project/session grouping, tab routing, search, and background opens. |
