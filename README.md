...
Target root: `<repo-root>/packages/app`, refreshed against commit `9cdc86dc35` on 2026-09-16. Story-only sources and
...
82 source files (including tests) share a relative path with the main app, 25 byte-for-byte identical. All
...
The September 16 pass consolidates cursor reads and restoration in `composer/editor/dom.ts`, following the
main app's DOM boundary. Both the editor view and interaction controller use that local helper. Selection anchors,
missing-selection fallback, zero-width characters, and text-node restoration keep their existing behavior.
Attachment capture retains its separate range-start calculation and zero-width stripping.

Timeline pointer handling and scroll-to-end scheduling now belong to `session/timeline/interaction.ts`.
`screen.tsx` supplies the visible messages and connects the scroller and pointer callback. The old
`use-session-hash-scroll-to-end.ts` wrapper is removed; 7777 still has no routed hashes or visible history paging.
The nine-dialog window, header counter, reasoning toggle, `SET_DOCUMENT_TITLE`, and `manageModels` controls remain.
...
Shared source paths increased from 80 to 82. The 25 byte-for-byte matches exclude the two editor consumers that now
import the consolidated cursor helper; this pass aligns responsibilities while preserving compact behavior.
All five shared public assets remain identical. Validation passes `bun test` (222 tests), `bun run typecheck`, and
`bun run build`. An isolated browser fixture also passes 17 cursor and timeline-scroll checks using bundled production
modules, including nested scroll regions and the pointer-gesture pause.
