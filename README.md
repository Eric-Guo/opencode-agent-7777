...
This review was refreshed against `packages/app` commit `52fddfb674044e895cbecd2b69ec7b80198b9d90` on
2026-08-08. The main app's `newLayoutDesigns` setting is now fixed to `true`, and its legacy layout implementation has
...
The refresh found 48 7777 source paths with a same-relative-path counterpart in the main app, up from 47. Native V2
protocol types now come directly from `@opencode-ai/client/promise`, as they do in the main app, while `src/types.ts`
is limited to the legacy rendering records still required by the shared session UI. The newly added
`src/utils/question-form.ts`, corrected `src/pages/session/composer/session-request-tree.ts`, and aligned
`src/pages/session/composer/session-permission-dock.tsx` are byte-for-byte identical to their main-app counterparts.
Together with `src/components/dialog-select-model-search.ts`, `src/constants/file-picker.ts`,
`src/utils/search-keydown.ts`, and `src/utils/uuid.ts`, seven reviewed sources are now identical. Within the reviewed
