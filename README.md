...
Target root: `<repo-root>/packages/app`, refreshed against commit `dd2edb5aba` on 2026-09-05. Story-only sources and
...
64 source files (including tests) share a relative path with the main app, 28 byte-for-byte identical. All
...
The September 5 pass refreshes seven package-owned editor/attachment sources and tests against the target commit,
including Escape cancellation of attachment drags and structured mention insertion. Against that same target,
byte-for-byte source matches increased from 21 to 28; the earlier count used the September 3 target.
