...
| Global/server sync and event reduction | `src/context/server-sync.tsx`, `src/context/server-session.ts`, `src/context/permission.tsx` | `src/context/server-sync.tsx`, `src/context/global-sync/bootstrap.ts`, `src/context/global-sync/event-reducer.ts`, `src/context/global-sync/queue.ts`, `src/context/global-sync/session-load.ts`, `src/context/global-sync/session-cache.ts` | 7777 combines startup, default session creation, SSE subscription, message refresh, model refresh, permission refresh, and event handling in the sync/session files. The app keeps bootstrap, event reducers, queues, session loading, and cache behavior in `global-sync/*`. |
...
| Session page shell, layout, navigation, and header actions | `src/pages/session.tsx`, `src/components/session/session-header.tsx`, `src/context/local.tsx`, `src/constants/session.ts` | `src/pages/session.tsx`, `src/pages/session/session-layout.ts`, `src/pages/session/new-session-layout.ts`, `src/pages/session/use-session-hash-scroll.ts`, `src/components/session/session-header.tsx`, `src/context/tabs.tsx`, `src/utils/session-route.ts` | 7777 combines session initialization, auto-scroll, loading/empty states, new-session creation, persisted session ID, and header controls in the session page/header path. The app separates route/session layout, tab/session routing, hash scroll, and header behavior. |
| Theme preload and applied theme side effects | `src/theme-preload.js`, `vite.config.ts`, `src/utils/theme.ts` | `public/oc-theme-preload.js`, `src/theme-preload.test.ts`, `src/app.tsx` (`ThemeProvider`) | 7777 keeps a local theme preload copy under `src/` and inlines it through Vite, plus a standalone window background sync helper. The app keeps the preload source under `public/`, tests it, and applies theme side effects through `ThemeProvider` in `app.tsx`. |
