...
This review was refreshed against `packages/app` commit `a414c5b5c5f641671142c30d57f6c70d4d6c3340` on
2026-08-01. An unsuffixed main-app filename is treated as a claim that the responsibility matches, not merely that
...
The refresh found 46 7777 paths with a same-relative-path counterpart in the main app. Two of those boundaries,
`src/utils/session-message.ts` and its test, were added during this review by moving current-message normalization
out of the compact session cache. `src/components/dialog-select-model-search.ts`, `src/constants/file-picker.ts`,
`src/utils/search-keydown.ts`, and `src/utils/uuid.ts` remain byte-for-byte identical to their main-app counterparts.
