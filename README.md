...
This review was refreshed against `packages/app` commit `fce8e0195c41887f40c459329646e10d064b3d34` on
2026-08-12. The main app's `newLayoutDesigns` setting is fixed to `true`, and its legacy layout implementation has
...
The refresh found 55 7777 source-tree paths with a same-relative-path counterpart in the main app, down from 56, and
23 of those files are byte-for-byte identical, up from 22. The lower counterpart count is intentional rather than a
regression: the main app's `src/hooks/provider-catalog.ts` now chooses between normalized global and directory stores,
while 7777 normalizes direct client responses, so the compact source and test now use the descriptive
`provider-catalog-client` stem. The exact shared set gained `src/pages/session/session-domain.ts` and caught up with the
current file-picker filter contract, deterministic message ordering, optimistic-turn insertion, and latest-assistant
error selection. 7777 also follows the shared prompt attachment blob-reference and UI pluralization contracts again,
while its serializable data-URL adapter remains explicitly local in `src/utils/draft-store-local.ts`. Within the
reviewed areas, no known file retains an unsuffixed main-app basename for a clearly different responsibility. The
remaining gaps below are product-scope differences rather than unresolved file moves.
...
| Timeline projection and rendering               | `src/pages/session/timeline/model.ts`, `src/pages/session/timeline/projection.ts`, `src/pages/session/timeline/rows.ts`, `src/pages/session/timeline/timeline-row.ts`, `src/pages/session/timeline/row-reconciliation.ts`, `src/pages/session/timeline/message-timeline.tsx`, `src/pages/session/timeline/summary-diffs.ts`                                                                                                                                                                                                                                                                                    | `src/pages/session/use-session-hash-scroll-to-end.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Grouped assistant-part construction, deterministic projection ordering, reconciliation, context-tool grouping, copy targeting, and latest-error selection follow the main app. The main app moved timeline I/O into session/timeline controllers; 7777 keeps compact projection assembly and cursor hydration in its model/cache. It still omits user-driven paging, virtualization, measurement, comment cards, and the full summary-diff UI. |
...
- `src/pages/session/timeline/timeline-row.ts`, `rows.ts`, `projection.ts`, `row-reconciliation.ts`, and
  ...
  projection can preserve shell turns, chronologically placed optimistic turns, stable context-group identity,
  interruption boundaries, recovered streaming turns, and grouped assistant parts.
...
- Main-app tests for the identical model-search, question-form, UUID, grouped-row projection, current-row
  construction, and summary-diff utilities are colocated under the same relative paths in 7777.
