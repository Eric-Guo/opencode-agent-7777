...
Target root: `<repo-root>/packages/app`, refreshed against commit `08330a1229` on 2026-09-07. Story-only sources and
...
66 source files (including tests) share a relative path with the main app, 26 byte-for-byte identical. All
...
| Shared leaf utilities | `src/runtime/persistence/base64.ts`, `src/runtime/platform/file-picker.ts`, `src/runtime/persistence/uuid.ts`, `src/runtime/server/errors.ts`, `src/shell/commands/search-keydown.ts`, shared `@opencode/schema/session-message` | `src/shell/errors/readable.ts` | Shared leaf boundaries stay local; the shared schema mints explicit message IDs. |

The September 7 pass replaces the combined model store with the main app's model and selection boundaries and
refreshes the package-owned editor and provider ordering. The toolbar now scrolls model controls independently of
the attachment and send controls, with edge fades in both text directions. Against the September 7 target, shared
source paths increased from 64 to 66 and byte-for-byte matches from 23 to 26; the previous review's 28 matches used
the September 5 target. Model regression tests cover fallback priority, persisted visibility, default isolation, and
bounded recency. The compact implementation still keeps one model selection and one catalog in its session store.
