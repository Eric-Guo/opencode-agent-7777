...
Target root: `<repo-root>/packages/app`, refreshed against commit `1d4e12e19f` on 2026-08-25. Story-only sources and
...
copies. 48 source files share a relative path with the main app, 23 byte-for-byte identical. All five public files
...
| Prompt input and composer | `src/composer/attachments/`, `src/composer/editor/`, `src/composer/suggestions/machine.ts`, `src/composer/types.ts`, `src/composer/prompt-parts.ts`, `src/composer/comment-note.ts`, `src/composer/prompt.ts`, `src/session/composer/session-composer-region-controller.ts`, `src/session/composer/session-composer-region.tsx` | `src/composer/adapter-compact.ts`, `src/composer/composer-compact.tsx`, `src/composer/state-compact.ts`, `src/composer/persistence-singleton.ts`, `src/composer/submit-compact.ts`, `src/runtime/persistence/drafts-local.ts` | Package-local composer editor with commands, context, shell mode, and routed/per-tab state disabled; one localStorage draft. The attachment module uses native listener cleanup instead of the main app's helper dependency. |
